...
int insertHeadDLL(doubleLL **list, void *data) {
    ...
    oldHead->prev = newHead; // Update previous pointer
...
/*
 (int) containsDLL takes a doubly linked list (doubleLL **) list and searches
 for the (void *) target item using a given (int) search (void *, void *)
 function to perform an "EqualsTo" check among data elements.

 If an item is found in the list, 1 is returned. Otherwise, 0 is returned.
*/
int containsDLL(doubleLL **list, void *target, int search (void *, void *)) {
    if (!list || !(*list)) return 0; // Check if passed pointers are valid
    doubleLL *current = *list;
    int found = 0;
    while (current) {
        // Check if current node is viable using given search function
...
/*
 (int) pathToDLL takes a (doubleLL **) list, a (void *) target, and an
 (int) search (void *, void *) function to perform an "EqualsTo" check on.
 From left to right, returns the index of a specific item if exists. Linked list
 walk starts at the node pointed to directly by (doubleLL **list).

 On success, the "index" of the given (void *) item in the linked list is
 returned. Otherwise, a negative error number is returned.
*/
int pathToDLL(doubleLL **list, void *target, int search (void *, void *)) {
    if (!list) return DLL_PATH_FAIL; // Check if passed pointers are valid
    doubleLL *current = *list;
    int found = 0;
    int index = 0;
    while (current) {
    ...
    return found ? index : DLL_EMPTY_FAIL;
}
